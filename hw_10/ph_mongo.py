...
class MongoConnact():
    ...
    def change_phn_bd(self, phones):
        ...
        ContactUser.objects(name=self.name).update(pull__phone=(cur_phone))
        ContactUser.objects(name=self.name).update(push__phone=(new_phone))
    
    def remove_name_bd(self):
        ContactUser.objects(name=self.name).delete()
    
    def delete_addr_bd(self):
        ContactUser.objects(name=self.name).update(address=Address(addr=None))

    def delete_brth_bd(self):
        #date_brth = datetime.strptime(self.birthday, '%d.%m.%Y').date()
        ContactUser.objects(name=self.name).update(birthday=Birthday(brth=None))
    
    def delete_eml_bd(self):
        ContactUser.objects(name=self.name).update(email=Email(eml=None))
    
    def delete_phn_bd(self, phone):
        ContactUser.objects(name=self.name).update(pull__phone=(phone))
    ...
    def srch_bd(self, command_line): 
        ...
        result = list(result)#one time iteration possible
        if result:
            return self.bd_print(result)
        else: 
            return 'No records find'
    
    def shw_bd(self):
        res = self.db.contact_user.find({})
        res = list(res)#one time iteration possible
        if res:
