...
engine = create_engine("sqlite:///myadd_book.db")
...
def add_name(command_line):
    if command_line:
        ...
        if name in contacts.keys():
            ...
            stmt = select(Contact.id, Contact.con_name).filter_by(con_name=name)
            result = session.execute(stmt).all()
            if not result:
                contact_name = Contact(con_name=name)
                session.add(contact_name)
                session.commit()
...
def get_cont_id(name):
    ...
    con_id = session.execute(stmt).scalar()
    return con_id
...
def add_email(command_line):
    ...
    con_id = get_cont_id(key)
    if con_id:
        stmt = update(Contact).where(Contact.id==con_id).values(email=input_email)
...
def delete_phone(command_line):
    ...
    if phone in contacts.get_record(key).phones_list:
        ...
        stmt = select(Phone.id, Contact).join(Phone).filter(Contact.con_name==key, Phone.description==phone)
        phone_id = session.execute(stmt).scalar()
        if phone_id:
    ...
    if re.search('\(0\d{2}\)\d{3}-\d{2}-\d{2}', phones[1]):
        if phones[0] in contacts.get_record(key).phones_list:
            ...
            if ix >= 0:
                ...
                stmt = select(Phone.id, Contact).join(Phone).filter(Contact.con_name==key, Phone.description==phones[0])
                phone_id= session.execute(stmt).scalar()
                if phone_id:
...
def find_note(command_line):
    ...
    new_msg = "\n".join(to_show)
    note = Note(new_msg + '\n' + msg)
    return ToConsole().to_outpute(note)
...
@input_error
def bd_print(res_stmt): 
    to_show = []
    for con in res_stmt: 
        con_id = con[0]
        stmt_p = select(Phone.description).filter_by(contact_id=con_id)
        phones_list = session.execute(stmt_p).scalars().all()
...
def show_bd(command_line): #new fun
    res = session.query(Contact.id, Contact.con_name, Contact.email).all()
    if res:
        return bd_print(res)
    else: 
...
def search_bd(command_line): #new fun
    key = ' '.join(command_line).strip()
    res = session.query(Contact.id, Contact.con_name, Contact.email).\
