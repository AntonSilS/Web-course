...
'''from models import Contact, Address, Birthday, Email, Phone
from models import db_session
from sqlalchemy.engine import create_engine
from sqlalchemy.orm import sessionmaker
from sqlalchemy.sql import select, update, delete, or_'''

#from models_2_2 import Address, Birthday, ContactUser, Email, PhoneUser
from models_2_2 import ContactUser
...
class MongoConnact():
    ...
    def add_addr_bd(self):
        ContactUser.objects(name=self.name).update(address=self.address)

    def add_brth_bd(self):
        #date_brth = datetime.strptime(self.birthday, '%d.%m.%Y').date()
        ContactUser.objects(name=self.name).update(birthday=self.birthday)

    def add_eml_bd(self):
        ContactUser.objects(name=self.name).update(eml=self.email)
    ...
    def bd_print(self, conts):
        ...
        for con in conts:
            ...
            con_name = con.name
            con_email = "____" if con.email == None else con.email.eml
            con_brth = "____" if con.birthday == None else con.birthday.brth
            con_addr = "____" if con.address == None else con.address.addr
            to_show.append(f'\n┌{"-" * 108}┐\
    ...
    def srch_bd(self, command_line): 
        ...
        if result:
            res = [cont['_id'] for cont in result]
            res_2 = [cont['_id'] for cont in result]
        ...
        '''users = ContactUser.objects.search_text(key)
        for i in users:
            print(i.name)'''
        
    
    def shw_bd(self):
        res = ContactUser.objects
        if res: 
...
class SqlLiteConnact():
    ...
    def add_addr_bd(self):
        ...
        if con_id:
            stmt = Address(brth=self.address, contact_id=con_id)
    ...
    def add_brth_bd(self):
        con_id = self.get_cont_id(self.name)
        if con_id:
            stmt = Birthday(brth=self.birthday, contact_id=con_id)
            self.session.execute(stmt)
            self.session.commit()
    ...
    def add_eml_bd(self):#work with instance of Record from contacts(ins. AddressBook)
        con_id = self.get_cont_id(self.name)
        if con_id:
            stmt = Email(eml=self.email, contact_id=con_id)
            self.session.execute(stmt)
            self.session.commit()
    ...
    def delete_addr_bd(self):
        con_id = self.get_cont_id(self.name)
        if con_id:
            stmt = delete(Address).where(Address.contact_id==con_id)
            self.session.execute(stmt)
    
    def delete_brth_bd(self):
        con_id = self.get_cont_id(self.name)
        if con_id:
            stmt = delete(Birthday).where(Birthday.contact_id==con_id)
            self.session.execute(stmt)
    
    def delete_eml_bd(self):
        con_id = self.get_cont_id(self.name)
        if con_id:
            stmt = delete(Email).where(Email.contact_id==con_id)
            self.session.execute(stmt)
            self.session.commit()
